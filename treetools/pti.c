...
int main(int argc, char **argv)
{
  ...
  while ((oc = getopt(argc, argv, "hue1rnkBdi:o:a:f:w:l:m:s:t:b:c:y:")) != -1)
  {
    switch (oc)
    {
    case 'y':
    ...
    case 'h':
      ...
      printf("-d: Plot all nodes as dots\n");
  ...
  else
  {
    ...
    while (!feof(infile))
    {
	  ...
	  /* fprintf(outfile, "gsave 1 0 0 setrgbcolor %f %f moveto %f 0.0 rlineto 0.0 %f rlineto %f 0.0 rlineto closepath stroke grestore\n", 200.0, 100.0, page_width, page_height - 100, -page_width); */
          if (unrooted_style)
            phyl_ps_utree(outfile, &phyltree, 200.0, 100.0, page_width, page_height - 100, &phyl_psinfo);
          else
            phyl_pstree(outfile, &phyltree, 200.0, 100.0, page_width, page_height - 100, &phyl_psinfo);
